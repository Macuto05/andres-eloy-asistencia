...
import { MatriculaConfig, DiasHabiles, RegistroMensual } from "@prisma/client"
...
export default function MatriculaClient({ matriculaData, diasHabiles, registros }: MatriculaClientProps) {
    ...
    // For Modals, we need to track both:
    // 1. The specific grade (grado) for the record (e.g. "1er Grado")
    // 2. The group level (nivel) (e.g. "Primaria")
    const [selectedGrado, setSelectedGrado] = useState("")
    const [selectedSeccionGroup, setSelectedSeccionGroup] = useState("")

    // Local state for optimistic updates
    // Key will be the 'grado' (e.g. "1er Grado") as that is unique enough across grouping if names are distinct.
    // Ideally, MatriculaConfig has 'grado' unique.
    const [inputValues, setInputValues] = useState<Record<string, number>>({})

    // Initialize inputs when Month or Registros change
    useEffect(() => {
        const initialValues: Record<string, number> = {}
        matriculaData.forEach(p => {
            // Find record matching: Mes, Tipo=Estudiante, Nivel=p.grado
            // Note: We intentionally match RegistroMensual.nivel to MatriculaConfig.grado
            const match = registros.find(r =>
                r.mes === selectedMonth &&
                r.nivel === p.grado
            )
            initialValues[p.grado] = match?.asistenciaMensual || 0
        })
        setInputValues(initialValues)
    }, [selectedMonth, registros, matriculaData])

    // Get Dias Habiles for selected month
    const diasHabilesDelMes = diasHabiles.find(d => d.mes === selectedMonth)?.cantidad || 0

    const handleInputChange = (grado: string, val: string) => {
        const num = parseInt(val) || 0
        setInputValues(prev => ({ ...prev, [grado]: num }))
    }

    const handleBlur = async (grado: string) => {
        const valor = inputValues[grado] || 0
        // Save to DB
        // Tipo = "Estudiante"
        // Nivel = p.grado (e.g. "1er Grado")
        await saveAsistenciaMensual(selectedMonth, "Estudiante", grado, valor)
    }

    const openAddModal = (grado: string, group: string) => {
        setSelectedGrado(grado)
        setSelectedSeccionGroup(group)
        setIsAddModalOpen(true)
    }

    const openViewModal = (grado: string, group: string) => {
        setSelectedGrado(grado)
        setSelectedSeccionGroup(group)
        setIsViewModalOpen(true)
    }

    // Helper to render a Table for a specific Group (Inicial, Primaria, etc.)
    const renderTable = (groupTitle: string, items: MatriculaConfig[]) => {
        if (items.length === 0) return null

        return (
            <div className="mb-10 last:mb-0">
                <h2 className="text-lg font-bold text-gray-700 mb-3 border-l-4 border-blue-500 pl-3 uppercase tracking-wide">
                    {groupTitle}
                </h2>
                        ...
                        <table className="w-full text-left border-collapse min-w-[1000px]">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                                    <th className="py-3 pl-4">Grado / Año</th>
                                    <th className="py-3 text-center">Secciones</th>
                                    ...
                                    <th className="py-3 text-center">Inasistencia Total</th>
                            ...
                            <tbody className="divide-y divide-gray-50 text-sm text-gray-700">
                                {items.map((item) => {
                                    ...
                                    const asistenciaInput = inputValues[item.grado] ?? 0
                                    ...
                                    return (
                                        <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                                            <td className="py-4 pl-4 font-medium text-gray-900 align-middle">{item.grado}</td>
                                            <td className="py-4 text-center text-gray-500 align-middle">{item.cantidadSecciones}</td>
                                                ...
                                                <div className="relative flex justify-center items-center gap-1 mx-auto h-full">
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            if (val === "") {
                                                                handleInputChange(item.grado, "0")
                                                                return
                                                            }
                                                            const num = parseInt(val)
                                                            if (!isNaN(num)) {
                                                                const clamped = Math.min(Math.max(0, num), targetAsistencia)
                                                                handleInputChange(item.grado, clamped.toString())
                                                            }
                                                        }}
                                                        onBlur={() => handleBlur(item.grado)}
                                                    />
                                            ...
                                            <td className="py-4 text-center align-middle">
                                                <div className="flex justify-center space-x-2">
                                                    <button
                                                        onClick={() => openAddModal(item.grado, item.nivel)}
                                                    ...
                                                    <button
                                                        onClick={() => openViewModal(item.grado, item.nivel)}
                                                        className="px-3 py-1 border border-gray-300 rounded-md text-xs font-medium hover:bg-gray-50 text-gray-700 transition-colors"
                                                    >
                                                        Ver
                                                    </button>
    ...
    // Grouping Data
    const inicialItems = matriculaData.filter(i => i.nivel === "Inicial")
    const primariaItems = matriculaData.filter(i => i.nivel === "Primaria")
    const mediaItems = matriculaData.filter(i => i.nivel === "Media General")
    ...
    return (
        <div className="w-full relative">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800">Matrícula General</h1>
            </div>


            {/* Selector de Meses */}
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-8 overflow-x-auto">
            ...
            {matriculaData.length === 0 ? (
                            ...
                            <tbody>
                                <tr>
                                    <td colSpan={11} className="py-8 text-center text-gray-500 italic">
                                        No hay grados configurados en la matrícula.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <>
                    {renderTable("Inicial", inicialItems)}
                    {renderTable("Primaria", primariaItems)}
                    {renderTable("Media General", mediaItems)}
                </>
            )}
            ...
            <div className="flex justify-end mt-8">
                <button
                    onClick={() => {
                        const reportTitle = "REPORTE DE ASISTENCIA - MATRÍCULA GENERAL"
                        const period = selectedMonth.toUpperCase()
                        const filename = `asistencia_matricula_${selectedMonth.toLowerCase()}_${new Date().getFullYear()}.csv`

                        const headers = ["Nivel", "Grado/Año", "Secciones", "Varones", "Hembras", "Total", "Días Hábiles", "Asistencia del Mes", "Promedio", "Porcentaje", "Inasistencia Total"]

                        const allItems = [...inicialItems, ...primariaItems, ...mediaItems]

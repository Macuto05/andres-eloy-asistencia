...
export default function AddInasistenciaModal({ isOpen, onClose, nivel, mes, cargo }: AddInasistenciaModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (tipoInasistencia === "periodo") {
            if (!fechaFin) {
                alert("Debes seleccionar una fecha de reintegro")
                setIsSubmitting(false)
                return
            }
            if (new Date(fechaFin) <= new Date(fechaInicio)) {
                alert("La fecha de reintegro debe ser posterior a la fecha de inicio.")
                setIsSubmitting(false)
                return
            }
        }
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-xs font-semibold text-gray-700 uppercase tracking-wider">Nombre</label>
                            <input
                                type="text"
                                required
                                value={nombre}
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-semibold text-gray-700 uppercase tracking-wider">
                            {tipoInasistencia === "dia" ? "Fecha" : "Fecha de Inicio"}
                        </label>
                        <input
                            type="date"
                            required
